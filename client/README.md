...
启动器右上角的 `⚙` 统一提供使用指引、中英文切换、声音开关、服务器设置、全解锁存档覆盖、网络修复和诊断导出。服务器设置可填写域名或 IP 与隧道端口，并单独指定更新服务端口；输入会先经过格式和端口范围校验，保存后 Launcher 会完整停止 Route Guard 与 EasyTier，再自动重启并应用新地址。公告、完整客户端检查和组件清单统一使用该更新端口。

“覆盖全解锁存档”与首次旧存档提示使用同一安全流程：连续两次倒计时确认，确认后先将现有存档备份到客户端 `backup_saves/时间戳/`，再写入内置存档。游戏运行时禁止覆盖存档或执行网络修复。
...
服务端只公布一个当前正式客户端版本，不使用“最低客户端版本”。普通启动时本地版本必须
与服务器完全一致，否则先安装服务器公布的完整包。开发测试时建议复制一份独立客户端目录，
使用 `SplinterCellCNLauncher.exe --test` 并连接本地测试服务器；只有本地服务器明确开启
“允许本地测试版客户端”后，较高版本才不会被正式包替换。正式服务器默认不接受该例外。
