...
impl CnAuthConfig {
    fn into_config(self) -> Config {
        ...
        // 国内专用 DLL 默认启用网络 IP 相关 Hook。
        // 这里即使 hooks.rs 里当前是强制启用，也保留这个配置，方便后续维护。
...
fn parse_cn_or_standard_config(content: &str) -> anyhow::Result<Config> {
    match toml::from_str::<CnAuthConfig>(content) {
        ...
        Err(cn_err) => {
            // 这里只是兼容同一个 5th_auth.dat 里写成原项目标准结构的情况。
            // 不会去读取 uplay.toml。
