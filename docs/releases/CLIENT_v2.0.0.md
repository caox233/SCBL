...
- 将使用指引、中英文切换和声音开关收纳到右上角 `⚙` 菜单，并新增服务器地址、隧道端口和更新服务端口设置；输入校验通过后按机器保存并自动安全重启。
- 设置菜单新增全解锁存档覆盖、网络修复和诊断导出；手动覆盖存档继续使用两次倒计时确认并先行备份，服务器设置保存后会自动安全重启 Launcher。
- 公告、完整客户端检查和组件清单统一读取用户配置的更新端口，删除未接入界面的旧快速复用、退出网卡清理和无效状态分支。
- 完整包写入四个组件的基线版本；客户端只下载服务端清单中版本更高的单个组件，同版本不再重复下载。
- 客户端组件始终以 SCBL 服务端为权威，不直连 GitHub；服务端可从 GitHub 单组件 Release 或本地上传取得文件。
