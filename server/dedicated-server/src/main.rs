...
const DEFAULT_MP_DATA: &str = include_str!("../assets/mp_balancing.ini");
...
fn start_server(logger: &slog::Logger, ctx: &Context, storage: &Arc<Storage>, is_secure: bool) -> io::Result<()> {
    ...
    if is_secure {
        // Only the secure/game connection owns online presence and room state.
        // The short-lived authentication connection must not clear an active
        // secure session when it disconnects or expires.
        server.expired_client_handler = Some(|ci: ClientInfo| {
            if let Some(user_id) = ci.user_id {
                match storage.close_secure_session(user_id) {
                    Ok(true) => info!(logger, "Preserving game state for API-active user after secure session expiry: {user_id}"),
                    Ok(false) => info!(logger, "Cleaned expired secure session of inactive user {user_id}"),
                    Err(e) => error!(logger, "session close error: {e}"),
                }
            }
        });
        server.disconnect_handler = Some(|ci: ClientInfo| {
            if let Some(user_id) = ci.user_id {
                match storage.close_secure_session(user_id) {
                    Ok(true) => info!(logger, "Preserving game state for API-active user after secure disconnect: {user_id}"),
                    Ok(false) => info!(logger, "Cleaned closed secure session of inactive user {user_id}"),
                    Err(e) => error!(logger, "session close error: {e}"),
                }
            }
        });
