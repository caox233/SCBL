...
/// A single UDP datagram is at most about 1 KiB. One MiB is intentionally
/// generous for valid compressed RMC data while preventing compression bombs.
const MAX_DECOMPRESSED_PACKET_BYTES: usize = 1024 * 1024;

fn decompress_payload(data: &[u8]) -> Result<Vec<u8>, Error> {
    decompress_to_vec_zlib_with_limit(data, MAX_DECOMPRESSED_PACKET_BYTES).map_err(Error::DecompressFailed)
}
        ...
        R: ReadBytesExt + std::io::Seek,
    {
        ...
        let use_compression = if !matches!(packet_type, PacketType::Syn) && matches!(source.stream_type, StreamType::RVSec) {
            ...
            if use_compression {
                ...
                payload = decompress_payload(&payload.as_slice()[1..])?;
...
\x60\xfd\x2a\x16\xfb\xb1\xab\x24\x87\x96\xfc\x3f\xcc\x7b\x5a\x7f";
    ...
    #[test]
    fn oversized_compressed_payload_is_rejected() {
        let expanded = vec![0u8; MAX_DECOMPRESSED_PACKET_BYTES + 1];
        let compressed = compress_to_vec_zlib(&expanded, 9);
        assert!(matches!(decompress_payload(&compressed), Err(Error::DecompressFailed(_))));
    }
